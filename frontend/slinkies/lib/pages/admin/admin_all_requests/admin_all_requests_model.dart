...
import '/index.dart';
import 'admin_all_requests_widget.dart' show AdminAllRequestsWidget;
...
class AdminAllRequestsModel extends FlutterFlowModel<AdminAllRequestsWidget> {
  ///  Local state fields for this page.

  List<RidesRecord> requestQuery = [];
  void addToRequestQuery(RidesRecord item) => requestQuery.add(item);
  void removeFromRequestQuery(RidesRecord item) => requestQuery.remove(item);
  void removeAtIndexFromRequestQuery(int index) => requestQuery.removeAt(index);
  void insertAtIndexInRequestQuery(int index, RidesRecord item) =>
      requestQuery.insert(index, item);
  void updateRequestQueryAtIndex(int index, Function(RidesRecord) updateFn) =>
      requestQuery[index] = updateFn(requestQuery[index]);

  bool isFullList = false;

  List<RidesRecord> searchedRides = [];
  void addToSearchedRides(RidesRecord item) => searchedRides.add(item);
  void removeFromSearchedRides(RidesRecord item) => searchedRides.remove(item);
  void removeAtIndexFromSearchedRides(int index) =>
      searchedRides.removeAt(index);
  void insertAtIndexInSearchedRides(int index, RidesRecord item) =>
      searchedRides.insert(index, item);
  void updateSearchedRidesAtIndex(int index, Function(RidesRecord) updateFn) =>
      searchedRides[index] = updateFn(searchedRides[index]);

  String? roleSelected;

  ///  State fields for stateful widgets in this page.

  // Stores action output result for [Firestore Query - Query a collection] action in admin-all_requests widget.
  VolunteersRecord? userLoggedIn;
  // Stores action output result for [Firestore Query - Query a collection] action in admin-all_requests widget.
  List<RidesRecord>? ridesQuery;
